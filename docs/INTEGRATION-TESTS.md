...
| `rooms` | Rooms in most cells, one unplaced room (Room schedule shows *Not Placed*), room separation lines, some rooms untagged, some tags pushed against a wall, one or two rooms "tagged" with a text note in a detail-line box (select it: it is a TextNote, not a room tag, and does not follow the room) | `Applied`; defect lines per unplaced/duplicate/untagged/awkward-tag room and a fake-tag line with the note and line ids; expected warnings *not in a properly enclosed region* / *Multiple Rooms are in the same enclosed region* dismissed and listed |
